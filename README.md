...
You can find practical code examples in the [examples/](examples/) directory:

- [BasicUsage.cs](examples/BasicUsage.cs) - Demonstrates minimal setup and a first download call.
- [AdvancedUsage.cs](examples/AdvancedUsage.cs) - Shows configuration, custom conversion settings, and error handling.
- [IntegrationExample.cs](examples/IntegrationExample.cs) - Illustrates how to wire the library into an ASP.NET DI container.

Additional examples covering batch processing, audio effects, diagnostics, and event handling are also available in that directory.
